...
async def get_prefix(bot: lightbulb.BotApp, message: hikari.Message) -> t.Union[t.Tuple[str], str]:
    ...
    """
    assert isinstance(bot, SamuroBot)
...
async def is_not_blacklisted(ctx: SamuroContext) -> bool:
...
class SamuroBot(lightbulb.BotApp):
    ...
    async def get_slash_context(
        ...
        cls: t.Type[lightbulb.SlashContext] = SamuroSlashContext,
    ) -> SamuroSlashContext:
    ...
    async def get_user_context(
        ...
        cls: t.Type[lightbulb.UserContext] = SamuroUserContext,
    ) -> SamuroUserContext:
    ...
    async def get_message_context(
        ...
        cls: t.Type[lightbulb.MessageContext] = SamuroMessageContext,
    ) -> SamuroMessageContext:
        return await super().get_message_context(event, command, cls)  # type: ignore

    async def get_prefix_context(
        self, event: hikari.MessageCreateEvent, cls: t.Type[lightbulb.PrefixContext] = SamuroPrefixContext
    ) -> t.Optional[SamuroPrefixContext]:
    ...
    async def on_started(self, event: hikari.StartedEvent) -> None:
        ...
        activity = hikari.Activity(name="@Samuro", type=hikari.ActivityType.LISTENING)
