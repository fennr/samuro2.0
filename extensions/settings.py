...
settings = SamuroPlugin("Settings")
...
class SettingsView(models.AuthorOnlyView):
    def __init__(
        ...
        *,
        ...
        self.input_event: asyncio.Event = asyncio.Evcaent()
...
Нажмите на одну из кнопок ниже, для настройки""",
    ...
    async def settings_report(self) -> None:
        """The reports menu."""
        assert isinstance(self.app, SamuroBot) and self.last_ctx is not None and self.last_ctx.guild_id is not None
...
async def ask_settings(
    ...
    *,
    ...
    if not invalid_select:
        ...
        assert isinstance(ctx.app, SamuroBot) and ctx.guild_id is not None
...
async def settings_cmd(ctx: SamuroSlashContext) -> None:
