...
misc = SamuroPlugin("Miscellaneous Commands")
...
async def ping(ctx: SamuroSlashContext) -> None:
    await ctx.respond(
        embed=hikari.Embed(
            title="🏓 Pong!",
...
@misc.command
@lightbulb.option("detach", "Отправить как отдельное сообщение", type=bool, default=True, required=False)
@lightbulb.option(
    "color",
...
@lightbulb.implements(lightbulb.SlashCommand)
async def embed(ctx: SamuroSlashContext) -> None:
    url_options = [
...
@lightbulb.implements(lightbulb.SlashCommand)
async def about(ctx: SamuroSlashContext) -> None:
    me = ctx.app.get_me()
    assert me is not None
...
@lightbulb.implements(lightbulb.SlashCommand)
async def setnick(ctx: SamuroSlashContext, nickname: t.Optional[str] = None) -> None:
...
@lightbulb.implements(lightbulb.SlashCommand)
async def support(ctx: SamuroSlashContext) -> None:
...
@lightbulb.implements(lightbulb.SlashCommand)
async def serverinfo(ctx: SamuroSlashContext) -> None:
    assert ctx.guild_id is not None
...
@lightbulb.implements(lightbulb.SlashCommand)
async def echo(ctx: SamuroSlashContext, text: str, channel: t.Optional[hikari.InteractionChannel] = None) -> None:
...
async def raw(ctx: SamuroMessageContext, target: hikari.Message) -> None:
    if target.content:
...
@lightbulb.implements(lightbulb.SlashCommand)
async def set_timezone(ctx: SamuroSlashContext, timezone: str) -> None:
